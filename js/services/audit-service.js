...
(function() {
    ...
    function matches(row, f) {
        ...
        if (f.desde && String(row.fecha || '').slice(0, 10) < f.desde) return false;
        if (f.hasta && String(row.fecha || '').slice(0, 10) > f.hasta) return false;
        return true;
    }

    function currentRows() {
        const f = filters();
        return read().filter(row => matches(row, f));
    }
    ...
    function renderBitacoraAuditoria() {
        ...
        const old = filters();
        const rows = rowsAll.filter(row => matches(row, old));
        ...
        cont.innerHTML = `
            <div style="display:flex;justify-content:space-between;gap:14px;align-items:flex-start;flex-wrap:wrap;margin-bottom:16px;">
                <div>
                    <h2 style="margin:0;color:#0f172a;">Bitacora de Auditoria</h2>
                    <p style="margin:4px 0 0;color:#64748b;">Rastro de operaciones sensibles, dinero y cambios relevantes.</p>
                </div>
            ...
            <div style="background:white;border:1px solid #e2e8f0;border-radius:8px;padding:14px;margin-bottom:14px;display:grid;grid-template-columns:2fr repeat(5, minmax(120px,1fr));gap:10px;align-items:end;">
                ...
                <div><label style="font-size:11px;font-weight:900;color:#475569;">DESDE</label><input type="date" id="audDesde" value="${esc(old.desde)}" onchange="renderBitacoraAuditoria()" style="width:100%;padding:9px;border:1px solid #cbd5e1;border-radius:6px;"></div>
                <div><label style="font-size:11px;font-weight:900;color:#475569;">HASTA</label><input type="date" id="audHasta" value="${esc(old.hasta)}" onchange="renderBitacoraAuditoria()" style="width:100%;padding:9px;border:1px solid #cbd5e1;border-radius:6px;"></div>
            </div>
    ...
    window.AuditService = { log, read, render: renderBitacoraAuditoria, requireAdmin };
