...
function recopilarNotificaciones() {
    ...
    cxp.filter(c => (c.saldoPendiente || 0) > 0 && c.fechaVencimiento).forEach(c => {
        ...
        if (fv <= en3dias) {
            ...
            if (!vistas.includes(id)) notifs.push({ tipo: 'cxp', icono: '💳', color: '#dc2626', msg: `Pago a ${c.proveedor || 'proveedor'}: ${dinero(c.saldoPendiente)} vence el ${fv.toLocaleDateString('es-MX', { day: '2-digit', month: '2-digit', year: 'numeric' })}`, id });
        }
    });
